...
path = Path("src/components/GlobePanel.tsx")
text = path.read_text()
...
old = """    for (const arc of activeArcs) {
...
new = """    for (const arc of activeArcs) {
      const positions = buildArcPositions(Cesium, arc)
      const color = Cesium.Color.fromCssColorString(FLOW_COLORS[arc.type] || '#8b949e')
...
if old not in text:
    raise SystemExit("Expected arc block not found; refusing to patch")
